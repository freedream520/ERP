...
namespace CZZD.ERP.SQLServerDAL
{
    public class SalesManage : ISales
    {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(" SELECT * FROM RECEIPT_ENTRY_VIEW ");
            if (!string.IsNullOrEmpty(strWhere))
            {
                strSql.Append("WHERE " + strWhere);
            }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.AppendFormat("update BLL_SALES  set STATUS_FLAG = {0}", CConstant.DELETE_STATUS);
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.AppendFormat("update BLL_RECEIPT_MATCH  set STATUS_FLAG = {0}", CConstant.DELETE_STATUS);
            strSql.Append(" where SLIP_NUMBER=@SLIP_NUMBER ");
            SqlParameter[] parameters = {
					new SqlParameter("@SLIP_NUMBER", SqlDbType.VarChar,50)};
            parameters[0].Value = SLIP_NUMBER;
        ...
        public decimal GetTotalDepositUnBilling(string customerClaimCode)
        {
            ...
            strSql.AppendFormat(" WHERE (BD.STATUS_FLAG <> {0}) AND (BS.STATUS_FLAG <> {1} OR ", CConstant.DELETE_STATUS, CConstant.DELETE_STATUS);
            strSql.AppendFormat(" BS.STATUS_FLAG IS NULL) AND (BRM.STATUS_FLAG <> {0} OR ", CConstant.DELETE_STATUS);
