...
namespace CZZD.ERP.SQLServerDAL
{
    public class ShipmentManage : IShipment
    {
        ...
        /// </summary>
        /// <returns></returns>
        private string GetShipmentMaxSlipNumber(string companyCode)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(" select isnull(max(convert(int,isnull(SLIP_NUMBER,0))),0) from BLL_SHIPMENT ");
            strSql.Append(" where COMPANY_CODE=@COMPANY_CODE");
