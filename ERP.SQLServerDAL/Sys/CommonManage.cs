    ...
    public class CommonManage : ICommon
    {
        ...
        public BaseMaster GetBaseMaster(string tableName, string code)
        {
            ...
            if ("DELIVERY".Equals(tableName))
            {
                ...
                strSql.AppendFormat(" where DELIVERY_CODE=@CODE and STATUS_FLAG <> {0} ", CConstant.DELETE_STATUS);
            }
            else
            {
                strSql.AppendFormat(" select CODE ,NAME from BASE_{0} ", tableName);
                strSql.AppendFormat(" where CODE=@CODE and STATUS_FLAG <> {0} ", CConstant.DELETE_STATUS);
            }
        ...
        public int GetMasterRecordCount(string tableName, string code, string name, string strWhere)
        {
            ...
            if ("DELIVERY".Equals(tableName))
            {
                ...
                strSql.AppendFormat(" where CUSTOMER_CODE like @CODE and DELIVERY_CODE like @NAME and  STATUS_FLAG <> {0}", CConstant.DELETE_STATUS);
            }
            else
            {

                strSql.AppendFormat("select count(1) from BASE_{0} ", tableName);
                strSql.AppendFormat(" where CODE like @CODE and NAME like @NAME and  STATUS_FLAG <> {0}", CConstant.DELETE_STATUS);
        ...
        public DataSet GetMasterList(string tableName, string code, string name, string strWhere)
        {
            ...
            else if ("SLIP_TYPE".Equals(tableName))
            {
                ...
                strSql.Append(" order by INDICATES_ORDER");
            }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.AppendFormat(" SELECT * FROM NAMES WHERE CODE_TYPE = '{0}' AND STATUS_FLAG <> {1} ORDER BY CODE ", codeType, CConstant.DELETE_STATUS);
