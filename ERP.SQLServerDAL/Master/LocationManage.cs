...
namespace CZZD.ERP.SQLServerDAL
{
    public class LocationManage : ILocation
    {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.AppendFormat("update BASE_LOCATION  set STATUS_FLAG = {0}", CConstant.DELETE_STATUS);
            strSql.Append(" where CODE=@CODE AND PRODUCT_CODE=@PRODUCT_CODE AND WAREHOUSE_CODE=@WAREHOUSE_CODE");
            SqlParameter[] parameters = {
					new SqlParameter("@CODE", SqlDbType.VarChar,50),
                    new SqlParameter("@PRODUCT_CODE", SqlDbType.VarChar,50),
        ...
        public DataSet GetList(string strWhere)
        {
            ...
            strSql.Append(" FROM base_location_view ");
            if (strWhere.Trim() != "")
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from base_location_view");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
