    ...
    public class UserManage : IUser
    {
        ...
        public bool Add(BaseUserTable model)
        {
            ...
            if (Exists(model.CODE, model.COMPANY_CODE))
            {
                ...
                parameters[9].Value = CConstant.NORMAL_STATUS;
            ...
            else
            {
                ...
                strSql.Append("CODE,PASSWORD,NAME,PHONE,EMAIL,DEPARTMENT_CODE,COMPANY_CODE,ROLES_CODE,PHOTO,STATUS_FLAG,CREATE_USER,CREATE_DATE,LAST_UPDATE_USER,LAST_UPDATE_TIME,INT_COMMUNITY_DATE,OUT_COMMUNITY_DATE)");
                strSql.Append(" values (");
                strSql.Append("@CODE,@PASSWORD,@NAME,@PHONE,@EMAIL,@DEPARTMENT_CODE,@COMPANY_CODE,@ROLES_CODE,@PHOTO,@STATUS_FLAG,@CREATE_USER,GETDATE(),@LAST_UPDATE_USER,GETDATE(),@INT_COMMUNITY_DATE,@OUT_COMMUNITY_DATE)");
                SqlParameter[] parameters = {
					new SqlParameter("@CODE", SqlDbType.VarChar,20),
					new SqlParameter("@PASSWORD", SqlDbType.VarChar,50),
					new SqlParameter("@NAME", SqlDbType.NVarChar,50),
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.AppendFormat("update BASE_USER  set STATUS_FLAG = {0}", CConstant.DELETE_STATUS);
            strSql.Append(" where CODE=@CODE and COMPANY_CODE=@COMPANY_CODE ");
            SqlParameter[] parameters = {
					new SqlParameter("@CODE", SqlDbType.VarChar,50),
					new SqlParameter("@COMPANY_CODE", SqlDbType.VarChar,50)};
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 CODE,PASSWORD,NAME,PHONE,EMAIL,DEPARTMENT_CODE,COMPANY_CODE,ROLES_CODE,PHOTO,STATUS_FLAG,CREATE_USER,CREATE_DATE,LAST_UPDATE_USER,LAST_UPDATE_TIME,DEPARTMENT_NAME,COMPANY_NAME,ROLES_NAME,INT_COMMUNITY_DATE,OUT_COMMUNITY_DATE from base_user_view ");
            strSql.Append(" where CODE=@CODE and COMPANY_CODE=@COMPANY_CODE ");
            strSql.AppendFormat(" and STATUS_FLAG <> {0}", CConstant.DELETE_STATUS);
            SqlParameter[] parameters = {
					new SqlParameter("@CODE", SqlDbType.VarChar,50),
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    model.INT_COMMUNITY_DATE = DateTime.Parse(ds.Tables[0].Rows[0]["INT_COMMUNITY_DATE"].ToString());
                }
                if (ds.Tables[0].Rows[0]["OUT_COMMUNITY_DATE"].ToString() != "")
                {
                    model.OUT_COMMUNITY_DATE = DateTime.Parse(ds.Tables[0].Rows[0]["OUT_COMMUNITY_DATE"].ToString());
                }
